...
**项目网址** ：http://101.43.53.65

🚀 **核心亮点**：
* 🤖 **AI 赋能**：集成 Spring AI，提供智能周边高性价比美食推荐、智能店铺风评总结、一键代写评价。
...
* 智能上下文对话与历史记录查询。
* 基于用户偏好的热门及附近商铺智能推荐。
* **语义检索 (RAG)**：集成 Qdrant 向量数据库，支持基于地理位置与用户偏好的深度语义搜索，解决传统 SQL 模糊查询无法处理的复杂语义需求。
* **自动化 Function Calling**：大模型根据用户自然语言自动调用后端接口，实现“对话即服务”的交互体验。
* **智能摘要与生成** ：自动汇总海量评价特征，为用户提供秒级的店铺“风评扫描”。
* **一键代写**：基于用户描述，渲染并代发布评价。
